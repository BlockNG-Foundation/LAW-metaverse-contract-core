...
contract LAWPUNK is IERC721 {
    ...
    string public imageHash =
        "02a5f13562b9fb74bc4a25032b546247839a89e3fbe9d96aeadede861440cb4b";

    string public baseURI;

    string internal nftName = "LAW Punks";
    string internal nftSymbol = "LPUNK";
    ...
    uint256 public tokenPrice = 1000 ether;
    ...
    function setTokenPrice(uint256 _tokenPrice) external onlyDeployer {
    ...
    function setBaseURI(string calldata _baseURI) external onlyDeployer {
        baseURI = _baseURI;
    }
    ...
    function mintB(uint256 quantity) external payable reentrancyGuard {
        require(!marketPaused);
        require(numSales.add(quantity) <= SALE_LIMIT, "Sale limit reached.");
        uint256 salePrice = price.mul(quantity);
    ...
    function mintT(uint256 quantity) external reentrancyGuard {
        require(!marketPaused);
        require(numSales.add(quantity) <= SALE_LIMIT, "Sale limit reached.");
        uint256 salePrice = tokenPrice.mul(quantity);
        IERC20(platformToken).transferFrom(
            msg.sender,
            address(this),
            salePrice
        );
    ...
    function aggregation(address _account)
        external
        view
        returns (
            uint256 total_,
        ...
        )
    {
        ...
        if (_account != address(0)) {
            balance_ = address(_account).balance;
            balanceToken_ = IERC20(platformToken).balanceOf(_account);
            allowance_ = IERC20(platformToken).allowance(
                _account,
                address(this)
            );
            balanceNFT_ = _getOwnerNFTCount(_account);
        }
    }
