...
contract CauldronV2Flat is BoringOwnable, IMasterContract {
    ...
    bool public onlyWhiteLiquidator = true;
    ...
    function init(bytes calldata data) public payable override {
        ...
        owner = masterContract.owner();
        liquidators[owner] = true;
    }
    ...
    function liquidate(
        ...
        if(onlyWhiteLiquidator){
            require(liquidators[msg.sender],"Emmm");
        }
    ...
    function setOnlyWhiteLiquidator(bool status) public onlyOwner {
       onlyWhiteLiquidator = status;
    }

    function setLiquidator(address addr ,bool status) public onlyOwner {
       liquidators[addr] = status;
    }
    ...
    function positionInfo(address user) public view
        returns (uint256 deposited, uint256 depositedShare, uint256 depositedValue, uint256 borrowed, uint256 borrowedPart, uint256 borrowedShare) {
        depositedShare = userCollateralShare[user];
        deposited = bentoBox.toAmount(collateral, depositedShare, false);
        depositedValue = deposited * 1e18 / exchangeRate;
        borrowedPart = userBorrowPart[user];
        borrowed = totalBorrow.toElastic(borrowedPart, true);
        borrowedShare = bentoBox.toShare(magicInternetMoney, borrowed, true);
    }
